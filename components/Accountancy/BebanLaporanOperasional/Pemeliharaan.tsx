...
import { calculateData, deletePemeliharaan, getPemeliharaan, storePemeliharaan } from '@/apis/Accountancy/BebanLaporanOperasional';
...
const Pemeliharaan = (data: any) => {
    ...
    const deleteSelectedData = () => {
        massDeleteData(selectedData, 'acc_blo_pemeliharaan').then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
                setSelectedData([]);
    ...
    const handleCalculate = () => {
        ...
        calculateData(periode?.id, year, 'pemeliharaan').then((res: any) => {
            if (res.status == 'success') {
                ...
                <div className="flex justify-end gap-4 items-center">
                    ...
                    {(dataInput.length > 0) && (
                        <DownloadButtons
                            ...
                            params={{
                                // type: instance ? 'pemeliharaan' : 'pemeliharaan_kab',
                                type: 'pemeliharaan',
                                category: 'beban_lo',
                                instance: instance,
                                periode: periode?.id,
