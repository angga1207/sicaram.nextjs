...
import { calculateData, deleteJasa, getJasa, storeJasa } from '@/apis/Accountancy/BebanLaporanOperasional';
...
const Jasa = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id && year) {
            getJasa(instance, periode?.id, year).then((res: any) => {
    ...
    const save = () => {
        setIsSaving(true);
        storeJasa(dataInput, periode?.id, year).then((res: any) => {
    ...
    const deleteSelectedData = () => {
        massDeleteData(selectedData, 'acc_blo_jasa').then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
                setSelectedData([]);
    ...
    const handleCalculate = () => {
        ...
        calculateData(periode?.id, year, 'jasa').then((res: any) => {
            if (res.status == 'success') {
