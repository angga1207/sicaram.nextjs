...
import { calculateData, deleteSubsidi, getSubsidi, storeSubsidi } from '@/apis/Accountancy/BebanLaporanOperasional';
...
const Subsidi = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id && year) {
            getSubsidi(instance, periode?.id, year).then((res: any) => {
    ...
    const save = () => {
        setIsSaving(true);
        storeSubsidi(dataInput, periode?.id, year).then((res: any) => {
    ...
    const handleCalculate = () => {
        ...
        calculateData(periode?.id, year, 'subsidi').then((res: any) => {
            if (res.status == 'success') {
