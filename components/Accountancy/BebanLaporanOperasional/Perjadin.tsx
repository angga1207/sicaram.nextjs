...
import { calculateData, deletePerjadin, getPerjadin, storePerjadin } from '@/apis/Accountancy/BebanLaporanOperasional';
...
const Perjadin = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id && year) {
            getPerjadin(instance, periode?.id, year).then((res: any) => {
    ...
    const save = () => {
        setIsSaving(true);
        storePerjadin(dataInput, periode?.id, year).then((res: any) => {
    ...
    const deleteData = (id: any) => {
        deletePerjadin(id).then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
    ...
    const handleCalculate = () => {
        ...
        calculateData(periode?.id, year, 'perjadin').then((res: any) => {
            if (res.status == 'success') {
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td className="border">
                                            <InputRupiah
                                                ...
                                                readOnly={data.editable ? false : !data.id ? false : true}
                                                onChange={(value: any) => {
                                                    setDataInput((prev: any) => {
                                                        const updated = [...prev];
                ...
                <div className="flex justify-end gap-4 items-center">
                    ...
                    {(dataInput.length > 0) && (
                        <DownloadButtons
                            ...
                            params={{
                                // type: instance ? 'perjadin' : 'perjadin_kab',
                                type: 'perjadin',
                                category: 'beban_lo',
                                instance: instance,
                                periode: periode?.id,
