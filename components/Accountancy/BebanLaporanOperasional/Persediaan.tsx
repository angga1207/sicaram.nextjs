...
import { calculateData, deletePersediaan, getPersediaan, storePersediaan } from '@/apis/Accountancy/BebanLaporanOperasional';
...
const Persediaan = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id && year) {
            getPersediaan(instance, periode?.id, year).then((res: any) => {
    ...
    const save = () => {
        setIsSaving(true);
        storePersediaan(dataInput, periode?.id, year).then((res: any) => {
    ...
    const handleCalculate = () => {
        ...
        calculateData(periode?.id, year, 'persediaan').then((res: any) => {
            if (res.status == 'success') {
                ...
                <div className="flex justify-end gap-4 items-center">
                    ...
                    {(dataInput.length > 0) && (
                        <DownloadButtons
                            ...
                            params={{
                                // type: instance ? 'persediaan' : 'persediaan_kab',
                                type: 'persediaan',
                                category: 'beban_lo',
                                instance: instance,
                                periode: periode?.id,
