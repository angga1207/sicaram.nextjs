...
import { faCloudDownloadAlt, faExclamationCircle, faLink, faPlus, faSave, faSpinner } from "@fortawesome/free-solid-svg-icons";
...
const KIB_A = (data: any) => {
    ...
    const _getDatas = () => {
        getKibA(instance, periode?.id, year).then((res) => {
    ...
    const _saveData = () => {
        setIsSaving(true);
        saveKibA(instance, periode?.id, year, dataInput).then((res) => {
            if (res.status === 'success') {
                setIsUnsaved(false);
