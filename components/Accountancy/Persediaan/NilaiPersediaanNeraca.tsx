...
import { deletePersediaanNeraca, getPersediaanNeraca, storePersediaanNeraca } from '@/apis/Accountancy/Persediaan';
...
const NilaiPersediaanNeraca = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id) {
            getPersediaanNeraca(instance, periode?.id, year).then((res: any) => {
    ...
    const updatedData = (data: any, index: number) => {
        setDataInput((prev: any) => {
            const updated = [...prev];
            updated[index].beban_hibah = updated[index].saldo_awal + updated[index].realisasi_lra + updated[index].hutang_belanja + updated[index].perolehan_hibah - updated[index].saldo_akhir;
            return updated;
        })
        setIsUnsaved(true);
    }
    ...
    const save = () => {
        ...
        storePersediaanNeraca(dataInput, periode?.id, year).then((res: any) => {
    ...
    const deleteData = (id: any) => {
        deletePersediaanNeraca(id).then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
...
export default NilaiPersediaanNeraca;
