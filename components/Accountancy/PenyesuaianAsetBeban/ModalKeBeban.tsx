...
import { deleteModalKeBeban, getModalKeBeban, storeModalKeBeban } from '@/apis/Accountancy/PenyesuaianAsetDanBeban';
...
const ModalKeBeban = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id) {
            getModalKeBeban(instance, periode?.id, year).then((res: any) => {
    ...
    const save = () => {
        setIsSaving(true);
        storeModalKeBeban(dataInput, periode?.id, year).then((res: any) => {
            if (res.status == 'success') {
    ...
    const deleteData = (id: any) => {
        deleteModalKeBeban(id).then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
    ...
    const deleteSelectedData = () => {
        massDeleteData(selectedData, 'acc_padb_modal_ke_beban').then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
                setSelectedData([]);
                ...
                <table className="table-striped">
                    <thead>
                        <tr className='left-0 sticky top-0 z-[1]'>
                            ...
                            <th className='bg-green-300 border text-center whitespace-nowrap'>
                                Aset Tetap Tanah
                            </th>
                            <th className='bg-green-300 border text-center whitespace-nowrap'>
                            ...
                            <>
                                {(index >= (page - 1) * perPage && index < (page * perPage)) && (
                                    <tr>
                                        ...
                                        <td className='bg-green-300 border'>
                                            {/* Beban Pegawai */}
                                            <InputRupiah
                                                isDisabled={isSaving == true}
                                                // readOnly={true}
