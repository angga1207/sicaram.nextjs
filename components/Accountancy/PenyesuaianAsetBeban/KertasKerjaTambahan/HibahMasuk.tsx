...
import { deleteBarjasKeAset, deleteHibahMasuk, getBarjasKeAset, getHibahMasuk, storeBarjasKeAset, storeHibahMasuk } from '@/apis/Accountancy/PenyesuaianAsetDanBeban';
...
const HibahMasuk = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id) {
            getHibahMasuk(instance, periode?.id, year).then((res: any) => {
    ...
    const save = () => {
        setIsSaving(true);
        storeHibahMasuk(dataInput, periode?.id, year).then((res: any) => {
    ...
    const _deleteData = (id: number) => {
        deleteHibahMasuk(id).then((res: any) => {
            if (res.status == 'success') {
    ...
    const deleteSelectedData = () => {
        massDeleteData(selectedData, 'acc_padb_tambahan_hibah_masuk').then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
                setSelectedData([]);
                                                        ...
                                                        {selectedMode ? (
                                                            <label className="w-12 h-6 relative">
                                                                <input type="checkbox" className="custom_switch absolute w-full h-full opacity-0 z-10 cursor-pointer peer" id="custom_switch_checkbox1"
                                                                    ...
                                                                    onChange={(e: any) => {
                                                                        if (e.target.checked) {
                                                                            setSelectedData((prev: any) => [...prev, item.id])
                                                                        } else {
                                                                            setSelectedData((prev: any) => prev.filter((selectedItem: any) => selectedItem != item.id))
                                                                        }
                                                                    }}
                                                                />
                                        ...
                                        {([9].includes(CurrentUser?.role_id) == false) && (
                                            <td className='border'>
                                                <Select
                                                    ...
                                                    value={
                                                        instances?.map((instance: any, index: number) => {
                                                            if (instance.id == item.instance_id) {
                                                                return {
                                                                    value: instance.id,
                                                                    label: instance.name,
                                                                }
