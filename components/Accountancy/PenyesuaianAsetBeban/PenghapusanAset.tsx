...
import { deleteAtribusi, deletePenghapusanAset, getAtribusi, getPenghapusanAset, storeAtribusi, storePenghapusanAset } from '@/apis/Accountancy/PenyesuaianAsetDanBeban';
...
const PenghapusanAset = (data: any) => {
    ...
    const _getDatas = () => {
        if (periode?.id) {
            getPenghapusanAset(instance, periode?.id, year).then((res: any) => {
    ...
    useEffect(() => {
        if (isMounted && dataInput.length > 0) {
            setTotalData((prev: any) => {
                ...
                updated.nilai_perolehan = dataInput.reduce((acc: any, curr: any) => acc + (parseFloat(curr.nilai_perolehan) || 0), 0);
    ...
    const save = () => {
        setIsSaving(true);
        storePenghapusanAset(dataInput, periode?.id, year).then((res: any) => {
    ...
    const deleteData = (id: any) => {
        deletePenghapusanAset(id).then((res: any) => {
            if (res.status == 'success') {
                _getDatas();
                ...
                <table className="table-striped">
                    <thead>
                        ...
                        <tr className='sticky top-[45px] left-0 z-[1] !bg-slate-900 !text-white'>
                            ...
                            <th className='whitespace-nowrap border border-slate-900 bg-green-300 text-slate-900'>
                                Persediaan
                            </th>
                            <th className='whitespace-nowrap border border-slate-900 bg-green-300 text-slate-900'>
                                Aset Tetap Tanah
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    setDataInput((prev: any) => {
                                                        ...
                                                        const data = [...prev];
                                                        data[index].nilai_perolehan = isNaN(value) ? 0 : value;
                                                        return data;
                                                    });
                                                }}
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td colSpan={3} className='border text-center p-3'>
                                {new Intl.NumberFormat('id-ID', { minimumFractionDigits: 0 }).format(totalData.total_data)} Item
                            </td>
                            <td className='border text-end p-3'>
                                Rp. {new Intl.NumberFormat('id-ID', { minimumFractionDigits: 0 }).format(totalData.nilai_perolehan)}
                            </td>
                            <td className='border text-end p-3'>
