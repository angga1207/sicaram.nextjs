...
import { getRefIndikatorTujuanSasaran, getDetailRefIndikatorTujuanSasaran, saveRefIndikatorTujuanSasaran, deleteRefIndikatorTujuanSasaran } from '@/apis/references';
...
const Index = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Referensi Indikator Tujuan & Sasaran'));
    });
    ...
    const getDetail = (id: any) => {
        getDetailRefIndikatorTujuanSasaran(id, type).then((res: any) => {
            if (res.status == 'success') {
                setDataInput({
                    inputType: 'edit',
                    id: res.data.id,
    ...
    const save = () => {
        setSaveLoading(true);
        saveRefIndikatorTujuanSasaran(dataInput).then((res: any) => {
            if (res.status == 'success') {
                setModalInput(false);
            ...
            <>
                <div className="flex flex-wrap gap-y-2 items-center justify-between mb-5 px-5">
                    <h2 className="text-xl leading-6 font-bold text-[#3b3f5c] dark:text-white-light xl:w-1/2 line-clamp-2 uppercase">
                        Referensi Indikator {type === 'tujuan' ? 'Tujuan' : 'Sasaran'}
                    </h2>
                    <div className="flex flex-wrap items-center justify-center gap-x-1 gap-y-2">

                        <div className="relative">
                            <form
                                className='flex items-center flex-col md:flex-row gap-2 gap-y-3'
                                onSubmit={
                                    (e) => {
                                        e.preventDefault();
                                ...
                                <div className="relative">
                                    <input type="search"
                                        ...
                                        placeholder={type === 'tujuan' ? 'Cari Indikator Tujuan...' : 'Cari Indikator Sasaran...'}
                                        onChange={(e) => {
                            ...
                            <thead>
                                <tr>
                                    <th className='!text-center border !min-w-[300px]'>
                                        Nama Indikator {type === 'tujuan' ? 'Tujuan' : 'Sasaran'}
                                    </th>
                                    ...
                                    <Dialog.Panel as="div" className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-[100%] md:max-w-[40%] my-8 text-black dark:text-white-dark">
                                        <div className="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                                            <h5 className="font-bold text-lg">
                                                ...
                                                {type === 'tujuan' ? ' Indikator Tujuan' : ' Indikator Sasaran'}
                                            </h5>
