...
import { getListProgramRenstraRenja, postRenstraNotes, saveRenstraKegiatan, saveRenstraSubKegiatan } from '@/apis/storedata';
import { fetchDetailRenstraKegiatan, fetchDetailRenstraSubKegiatan, fetchRenstra, fetchRenstraValidatorNotes, fetchSatuans } from '@/apis/fetchdata';
...
const Page = () => {
        ...
        if (isMounted && router.query.program && arrPrograms.length > 0) {
            ...
            if (selectProgram) {
                ...
                if (periode?.id && instanceId && isLoadingData === false) {
                    _fetchRenstra(periode.id, instanceId, selectProgram.id);
                }
            } else {
    ...
    const _fetchRenstra = (periodeId: any, instanceId: any, programId: any) => {
        setIsLoadingData(true);
        setDatas([]);
        setDataInformation(null);
        fetchRenstra(periodeId, instanceId, programId).then((data) => {
            if (data.status == 'success') {
                setDatas(data.data.datas);
                setDataInformation(data.data.renstra);
            }
        }).catch((error) => {
            console.error('Error fetching renstra data:', error);
            showAlert('error', 'Terjadi kesalahan saat mengambil data renstra');
        }).finally(() => {
    ...
    const openModalInput = (selected: any) => {
        if (selected.type == 'kegiatan') {
            fetchDetailRenstraKegiatan(periode?.id, instanceId, selected.program_id, selected.id, year).then((data) => {
                if (data.status == 'success') {
                    setDataInput({
                        ...data.data,
                        inputType: 'edit',
        ...
        if (selected.type == 'sub-kegiatan') {
            fetchDetailRenstraSubKegiatan(periode?.id, instanceId, selected.program_id, selected.id, year).then((data) => {
                if (data.status == 'success') {
                    setDataInput({
                        ...data.data,
                        inputType: 'edit',
    ...
    const save = () => {
        if (dataInput?.type == 'kegiatan') {
            saveRenstraKegiatan(periode?.id, instanceId, selectedProgram.id, dataInput?.id, year, dataInput).then((data) => {
                if (data.status == 'success') {
                    showAlert('success', data.message);
                    setUnsave(false);
                    setModalInput(false);
                    _fetchRenstra(periode?.id, instanceId, selectedProgram.id);
                }
        ...
        if (dataInput?.type == 'sub-kegiatan') {
            saveRenstraSubKegiatan(periode?.id, instanceId, selectedProgram.id, dataInput?.id, year, dataInput).then((data) => {
                if (data.status == 'success') {
                    showAlert('success', data.message);
                    setUnsave(false);
                    setModalInput(false);
    ...
    const _fetchLogs = () => {
        ...
        fetchRenstraValidatorNotes(periode?.id, instanceId, selectedProgram.id, dataInformation?.id)
            .then((data) => {
                if (data.status == 'success') {
    ...
    const sendMessage = () => {
        ...
        if (message && status) {
            ...
            postRenstraNotes(periode?.id, instanceId, selectedProgram.id, dataInformation?.id, message, status)
                .then((data) => {
                    if (data.status == 'success') {
                        showAlert('success', data.message);
                        fetchRenstraValidatorNotes(periode?.id, instanceId, selectedProgram.id, dataInformation?.id).then((data) => {
                            if (data.status == 'success') {
                                setDataLogs(data.data);
                                setUnsave(false);
                                document.getElementById('textarea-message')?.focus();
                        ...
                        // _fetchRenstra(periode?.id, instance, selectedProgram.id);
                        setMessage('');
                        setStatus('');
