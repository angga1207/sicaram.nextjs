...
const Index = () => {
    ...
    const addDataOnSpesific = (kegiatan_id: any) => {
        ...
        setDataInput({
            ...
            code: '',
            fullcode: '',
            parent_code: programs.filter((data) => data.id == kegiatan_id)[0].code ?? '',
            description: '',
        });
        setModalInput(true);
    }
    ...
    const changeKegiatan = (e) => {
        const Parent = kegiatans.filter((data) => data.id == e);
        ...
        if (dataInput.inputType == 'create') {
            storeSubKegiatan(dataInput).then((data) => {
                ...
                if (data.status == 'error validation') {
                    ...
                    document.getElementById('error-kegiatan_id').innerHTML = data.message.kegiatan_id?.[0] ?? '';
    ...
    const confirmDelete = async (id: number) => {
        ...
        swalWithBootstrapButtons
            ...
            .then((result) => {
                if (result.value) {
                    deleteSubKegiatan(id).then((data) => {
                        if (data.status == 'success') {
                            fetchSubKegiatans(periode, instance, search).then((data) => {
                                setDatas((data?.data));
                            });
                                        ...
                                        {datas.map((data) => {
                                            return (
                                                <tr className={data?.type != 'sub-kegiatan' ? 'cursor-pointer group relative bg-green-100 dark:bg-green-800' : 'cursor-pointer group relative hover:bg-slate-100 dark:hover:bg-slate-700'}>
                                                    ...
                                                    <td onClick={() => editSubKegiatan(data?.id)}>
                                                            ...
                                                            <>
                                                                <div className="">
                                                                    <select
                                                                        ...
                                                                        value={dataInput.kegiatan_id}
                                                                        onChange={(e) => changeKegiatan(e.target.value)}
                                                                    >
