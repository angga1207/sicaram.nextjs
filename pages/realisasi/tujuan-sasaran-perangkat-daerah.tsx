...
const Index = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Realisasi Tujuan & Sasaran Perangkat Daerah'));
    });
    ...
    useEffect(() => {
        if (isMounted && instance) {
            ...
            if (isMounted && periode?.id && year && instance) {
                getIndex(periode?.id, year, instance).then((data: any) => {
    ...
    }, [isMounted, periode?.id, year, instance])
    ...
    const editTujuan = (id: any, ref_id: any, month: number) => {
        ...
        getDetail('tujuan', id, ref_id, year, month, periode?.id, instance).then((data: any) => {
            if (data?.status == 'success') {
                setDataInput({
                    inputType: 'tujuan',
    ...
    const editSasaran = (id: any, ref_id: any, month: number) => {
        ...
        getDetail('sasaran', id, ref_id, year, month, periode?.id, instance).then((data: any) => {
            if (data?.status == 'success') {
                setDataInput({
                    inputType: 'sasaran',
            ...
            <>
                ...
                <div className="flex flex-wrap gap-y-2 items-center justify-between mb-5 px-5">
                    <h2 className="text-xl leading-6 font-bold text-[#3b3f5c] dark:text-white-light xl:w-1/2 line-clamp-2 uppercase">
                        Realisasi Tujuan & Sasaran Perangkat Daerah Tahun {year}
                    </h2>
